...
await publisher.ConnectAsync(configuration);

if(publisher.isConnected)
{
    while(true)
    {
        string message = InputValidator.ValidateInput("Enter topic: ", input => (!string.IsNullOrWhiteSpace(input), input), "Empty field!");
